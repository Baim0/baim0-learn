...
```bash
bash
复制代码$ tcpdump -i eth0 -nn -s0 -v port 80
```
...
```bash
bash
复制代码$ tcpdump -i eth0 host 10.10.1.1
```
...
```bash
bash
复制代码$ tcpdump -i eth0 dst 10.10.1.20
```
...
```bash
bash
复制代码$ tcpdump -i eth0 -s0 -w test.pcap
```
...
```bash
bash
复制代码$ tcpdump -i eth0 -s0 -l port 80 | grep 'Server:'
```
...
```bash
bash
复制代码$ tcpdump src host 1.2.3.4
```

### Network 过滤器

Network 过滤器用来过滤某个网段的数据，使用的是 [CIDR](https://link.juejin.cn?target=https%3A%2F%2Fen.wikipedia.org%2Fwiki%2FClassless_Inter-Domain_Routing) 模式。可以使用四元组（x.x.x.x）、三元组（x.x.x）、二元组（x.x）和一元组（x）。四元组就是指定某个主机，三元组表示子网掩码为 `255.255.255.0`，二元组表示子网掩码为 `255.255.0.0`，一元组表示子网掩码为 `255.0.0.0`。例如，

抓取所有发往网段 `192.168.1.x` 或从网段 `192.168.1.x` 发出的流量：

```bash
bash
复制代码$ tcpdump net 192.168.1
```
...
```bash
bash
复制代码$ tcpdump src net 172.16.0.0/12
```
...
```bash
bash
复制代码21:27:06.995846 IP (tos 0x0, ttl 64, id 45646, offset 0, flags [DF], proto TCP (6), length 64)
...
```bash
bash
复制代码$ tcpdump -nn -A -s1500 -l | grep "User-Agent:"
```
...
```bash
bash
复制代码$ tcpdump -nn -A -s1500 -l | egrep -i 'User-Agent:|Host:'
```
...
```bash
bash
复制代码$ tcpdump -s 0 -A -vv 'tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x47455420'
```
...
```bash
bash
复制代码$ tcpdump -s 0 -A -vv 'tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x504f5354'
```

注意：该方法不能保证抓取到 HTTP POST 有效数据流量，因为一个 POST 请求会被分割为多个 TCP 数据包。

上述两个表达式中的十六进制将会与 GET 和 POST 请求的 `ASCII` 字符串匹配。例如，`tcp[((tcp[12:1] & 0xf0) >> 2):4]` 首先会[确定我们感兴趣的字节的位置](https://link.juejin.cn?target=https%3A%2F%2Fsecurity.stackexchange.com%2Fquestions%2F121011%2Fwireshark-tcp-filter-tcptcp121-0xf0-24)（在 TCP header 之后），然后选择我们希望匹配的 4 个字节。
...
```bash
bash
复制代码$ tcpdump -s 0 -v -n -l | egrep -i "POST /|GET /|Host:"
...
```bash
bash
复制代码$ tcpdump -s 0 -A -n -l | egrep -i "POST /|pwd=|passwd=|password=|Host:"
...
```bash
bash
复制代码$ tcpdump -nn -A -s0 -l | egrep -i 'Set-Cookie|Host:|Cookie:'
...
```bash
bash
复制代码$ tcpdump 'icmp[icmptype] != icmp-echo and icmp[icmptype] != icmp-echoreply'
...
```bash
bash
复制代码$ tcpdump -nn -l port 25 | grep -i 'MAIL FROM\|RCPT TO'
```
...
```bash
bash
复制代码$ onesixtyone 10.10.1.10 public
...
```bash
bash
复制代码$ tcpdump -n -s0  port 161 and udp
tcpdump: verbose output suppressed, use -v or -vv for full protocol decode
listening on wlp58s0, link-type EN10MB (Ethernet), capture size 262144 bytes
...
```bash
bash
复制代码$ tcpdump  -w /tmp/capture-%H.pcap -G 3600 -C 200
```
...
```bash
bash
复制代码$ tcpdump -nr ipv6-test.pcap ip6 proto 17
```
...
```bash
bash
复制代码$ nmap -p 80 --script=http-enum.nse targetip
```
...
```bash
bash
复制代码$ tcpdump -nn port 80 | grep "GET /"
...
```bash
bash
复制代码$ tcpdump -i wlp58s0 -s0 port 53
...
```bash
bash
复制代码$ tcpdump 'tcp port 80 and (((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)'
```
...
```bash
bash
复制代码$ ssh root@remotesystem 'tcpdump -s0 -c 1000 -nn -w - not port 22' | /Applications/Wireshark.app/Contents/MacOS/Wireshark -k -i -
```
...
```bash
bash
复制代码$ ssh root@remotesystem 'tcpdump -s0 -c 1000 -nn -w - port 53' | /Applications/Wireshark.app/Contents/MacOS/Wireshark -k -i -
```

抓取到的数据：

![img](../img/20200210170101.png)
...
```bash
bash
复制代码$ tcpdump -nnn -t -c 200 | cut -f 1,2,3,4 -d '.' | sort | uniq -c | sort -nr | head -n 20
...
```bash
bash
复制代码$ tcpdump port http or port ftp or port smtp or port imap or port pop3 or port telnet -l -A | egrep -i -B5 'pass=|pwd=|log=|login=|user=|username=|pw=|passw=|passwd=|password=|pass:|user:|username:|password:|login:|pass |user '
```
...
```bash
bash
复制代码$ tcpdump -v -n port 67 or 68
